...
public class Order {
    ...
    public static void validProduct(List<OrderProduct> orderProducts) {
        if (orderProducts == null || orderProducts.isEmpty()) {
            throw new IllegalArgumentException("orderReqs invalid");
        }
    }

    public static void validCustomer(String customerName, String customerEmail) {
        if (customerName == null || customerEmail == null) {
            throw new IllegalArgumentException("customer info required");
        }
    }

    public BigDecimal calcShipPrice(BigDecimal subtotal) {
        return subtotal.compareTo(new BigDecimal("100.00")) >= 0 ? BigDecimal.ZERO : new BigDecimal("5.00");
    }

    public BigDecimal calcDiscount(String couponCode) {
